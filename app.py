...
def instructor_dashboard():
    ...
    if os.path.exists(doubt_file):
        df_doubts = pd.read_excel(doubt_file)
        doubt_count = len(df_doubts)

    all_courses = default_courses + dynamic_courses
    return render_template('instructor_dashboard.html', courses=all_courses, total=len(all_courses), doubt_count=doubt_count)
...
def create_quiz():
    ...
    if request.method == 'POST':
        ...
        # Add questions under selected course
        if course not in existing_questions:
            existing_questions[course] = []
        existing_questions[course].extend(questions)
    ...
    return render_template('create_quiz.html', courses=all_courses)

# ------------------------ Download Enrollments ------------------------
@app.route('/download_enrollments')
def download_enrollments():
    if session.get('role') != 'instructor':
        return "Access Denied", 403
    ...
    return "Enrollments file not found.", 404
...
@app.route('/download_quiz_results')
def download_quiz_results():
    if session.get('role') != 'instructor':
        return "Access Denied", 403
    ...
    if os.path.exists(file_path):
        return send_file(file_path, as_attachment=True)
    return "Quiz results file not found.", 404
...
def view_analytics():
    ...
    enrollments_path = os.path.join(app.config['UPLOAD_FOLDER'], 'enrollments.xlsx')
    quiz_results_path = os.path.join(app.config['UPLOAD_FOLDER'], 'quiz_results.xlsx')

    enrollments = {}
    quiz_summary = {}

    if os.path.exists(enrollments_path):
        df = pd.read_excel(enrollments_path)
        enrollments = df['Course'].value_counts().to_dict()

    if os.path.exists(quiz_results_path):
        df = pd.read_excel(quiz_results_path)
        quiz_summary = df['Level'].value_counts().to_dict()

    return render_template('view_analytics.html', enrollments=enrollments, quiz_summary=quiz_summary)
...
def view_course(course_title):
    ...
    # Load courses from default and dynamic sources
    default_courses = [
        {"title": "Introduction to Python", "description": "Learn the basics of Python programming"},
        {"title": "Web Development with Flask", "description": "Build web apps using Flask"},
        {"title": "Database Design", "description": "Master SQL and ER diagrams"}
    ...
    course = next((c for c in all_courses if c['title'] == course_title), None)
    if not course:
...
def view_doubts():
    ...
    doubts = []
    if os.path.exists(doubt_file):
        df = pd.read_excel(doubt_file)
        doubts = df.to_dict(orient='records')
